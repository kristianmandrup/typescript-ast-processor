...
export class FunctionLikeNodeTester extends DeclarationNodeTester {
  ...
  constructor(node : any, options : any) {
    ...
    if (parameters) {
      ...
      this.log('FunctionLikeTester: no typeParameters', {node})
    }
...
testName(query : any) {
  if (!query || !this.isNamed)
    return true
  return this.identifierNodeTester
    ? this
      .identifierNodeTester
      .testName(query)
    : false
}
...
/**
   ...
   */
test(query : any) {
  return this.testName(query.name) && this.testType(query.type) && // async, arrow or normal
  this.testParameters(query.parameters) && this.testReturnType(query.returnType)
}
